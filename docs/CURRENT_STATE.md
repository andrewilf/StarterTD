...
- ChampionWalling: Places wall segments (30 HP, 10k movement cost) adjacent to its network. Segments BFS-connected to any walling anchor are protected from decay; orphaned segments decay 1 HP/sec per exposed cardinal side (max 4 HP/sec). World-space "+" button toggles wall-placement mode; click-drag creates an AoE-style single-corner L path (preview while dragging, commit on release). If one corner route is blocked, the alternate corner is chosen when it yields a longer valid prefix; placement then stops at first invalid tile
