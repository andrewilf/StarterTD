...
- **Enemies**: Base class with state machine (Moving, Attacking). Follows path to exit. Detects towers blocking next waypoint and switches to Attacking state, dealing damage every 1 second. Resumes moving when tower dies or path updates. Visual feedback: red tint while attacking. Per-wave AttackDamage property scales with wave difficulty.
