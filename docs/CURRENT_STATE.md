...
- **Towers**: 2 Types (Gun, Cannon) with distinct movement costs for strategic maze control. Supports placement (L-Click) and Upgrading (R-Click). Per-type health (Gun: 100/120, Cannon: 150/180). Blocking capacity system: Gun (3 slots), Cannon (2 slots) — limits simultaneous enemy attacks. Health bar shown when damaged. Blue capacity bar shows remaining engagement slots. Destroyed towers clear tile occupancy reference and trigger pathfinding recomputation.
