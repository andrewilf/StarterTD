...
- Gum UI runtime: `Game1` owns `GumService.Default` init/update/resize plumbing. Gum controls are scene-owned roots attached/detached in scene load/unload (currently used by `StartMenuScene`)
