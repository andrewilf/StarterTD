...
- ChampionHealing ultimate (`TowerManager`): 15s support window that instantly refills all healing drones, removes drone healing energy cost, and applies a `1.3x` external attack-speed multiplier to all attacking towers (including walling). Towers under this multiplier render a separate white/gold sparkle aura. Cooldown is defined in stats (`ChampionHealing: 50s`). The ult is terminated early when the healing champion is sold or destroyed
