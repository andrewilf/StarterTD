...
- `TowerManager.BuildAttackZone(wallSet)`: shared helper returning all in-bounds tiles 1 step outside the connected wall set in all 8 directions. Used by `DrawWallRangeIndicatorForSet` (hover preview), `FindWallNetworkTarget` (targeting), and `UpdateWallFrenzy`. `FindWallNetworkTarget` prefers closest non-slowed enemy; falls back to closest slowed enemy
- `Tower.ApplyDecayDamage(float dt)`: accumulates fractional HP loss; used by wall segments decaying at 1 HP/sec
- Wall segments: `TowerType.WallSegment`, 30 HP, 10k movement cost, placed by `TowerManager.TryPlaceWall()`. Must be 4-directionally adjacent to the placing tower's connected wall set (BFS). `_wallConnectedSets: Dictionary<Tower, HashSet<Point>>` — rebuilt each `Update` as a single-root BFS per walling tower (champion or generic) so disconnected towers don't share attack zones. Used by placement validation, decay suppression, targeting, frenzy, and hover draw. `UpdateWallDecay()` unions all per-tower sets; segments in the union are protected, orphaned segments decay at 1 HP/sec per exposed cardinal side. Runs before the dead-tower sweep in `TowerManager.Update()`
...
- `TowerStats.AbilityEffect: Action<Tower>?`: delegate stored per tower type in its stats file. Called by `TowerManager.TriggerChampionAbility()` on the champion and all matching generics. For `ChampionWalling`, always buffs all Walling generics even if the champion is dead
...
- Champion death calls `UpdateChampionStatus(false)` on matching generics (including Walling generics)
...
- Walling: consolidated button same as Gun/Cannon — champion mode when dead, generic (cost 50) when alive. Wall segment placement uses world-space "+" button on any selected walling tower (champion or generic)
...
- `_wallPlacementMode` bool: toggled by clicking the 18×18 "+" world-space button top-right of any selected walling tower (champion or generic). Grid left-clicks call `TryPlaceWall()` — both types use the same `IsAdjacentToWallingNetwork` check against that tower's own connected set. Right-click move is blocked while active
...
- Hover feedback: `DarkGreen * 0.5f` if valid placement (buildable + adjacent to tower's network), `Red * 0.3f` otherwise; grid outline becomes `DarkGreen`
- Mode clears on: ESC, selecting another tower/enemy, UI panel click with `SelectedTowerType`, selling the tower
