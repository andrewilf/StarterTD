...
- ChampionHealing ultimate (`TowerManager`): timed support window that instantly refills all healing drones, removes drone healing energy cost, and applies an external attack-speed multiplier to all attacking towers (including walling). Towers under this multiplier render a separate white/gold sparkle aura. If the healing champion is in attack mode, ability cast applies no effect (cooldown still starts in `GameplayScene`). Active healing ult is terminated immediately when mode switches to attack, and also when champion is sold/destroyed (via `EndChampionUltEffects`)
